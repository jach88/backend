...
class ProductoModel(models.Model):
    ...
    class Meta:
        ...
        #db_table => indica el nombre de la tabla en la base de datos
        db_table = 'productos'
        #ordering => modifica el ordenamiento por defecto (x el id) al momento de devolver los registros
        # si queremos ascendente no le ponemos nada y si queremos descendente le ponemos el guion
        ordering = ['-productoPrecio']
        #verbose_name y verbose_name_plural=> sirven para el panel administrativo (CMS) de django
        verbose_name = 'producto'
        verbose_name_plural = 'productos'


class ClienteModel(models.Model):
    #si vamos a usar el panel administrativo del django entonces solo ahi se debe usar el verbose_name y el help_text
    clienteId = models.AutoField(
        db_column='id', primary_key='True',unique=True, null=False)
    
    clienteNombre = models.CharField(
        max_length=45, db_column='nombre', verbose_name='nombre',
        help_text='Ingresa aqui el nombre')
...
class CabeceraModel(models.Model):
    ...
    #related name sirve para ingresar desde la clase cliente model a todos sus registros de cabeceras
    #on_delete => es que accion tomara cuando un registro de esa FK sea eliminado
    ...
    #SET_NULL => elimina el cliente y todas sus cabeceras le cambia el valor a null
    ...
    clientes = models.ForeignKey(to=ClienteModel,db_column='clientes_id', 
                                 null=False, related_name='clienteCabeceras', on_delete=models.
                                 PROTECT)
