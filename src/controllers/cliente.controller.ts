...
import { paginationHelper, paginationSerializer } from "../utils/paginator.helper";
...
export const eliminarCliente =async(req: Request, res:Response)=>{
    ...
    try {
        ...
        //mediante el uso de un operador ternario indicar que si no hubo eliminacion, mostrar un status 404


        return res.status(clienteEliminado ? 200 : 404).json({
            content:clienteEliminado,
            message: clienteEliminado
            ?"Cliente eliminado exitosamente":"No se encontro al cliente",
        });
    } catch (error) {
